...
from config import SEARCH_URL, MAX_PAGES, HEADLESS, USER_AGENT
...
async def _get_card_images(card) -> list[str]:
...
async def _extract_listings(page) -> list[dict]:
    ...
    btn_only = page.locator(
        'li[data-cy="rp-property-cd"]:not(:has(a[href*="/imovel/"])) a[role="button"]'
    )
...
async def fetch_listings() -> list[dict]:
    ...
    async with async_playwright() as p:
        ...
        for page_num in range(1, MAX_PAGES + 1):
            ...
            if new_on_page == 0:
                print(f"  [scraper] no new listings on page {page_num}, stopping.")
                break
