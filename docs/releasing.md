...
1. **Abra um PR de release** a partir de `main` (`chore/release-X.Y.Z`) com duas mudanças:
   - `src/farejador/__init__.py` → `__version__ = "X.Y.Z"` (o `pyproject.toml` lê daqui);
...
- **Automático**: todo PR que mexe em `packaging/**`, `pyproject.toml`, `src/farejador/desktop.py`, `__main__.py` ou `__init__.py` dispara o workflow *Release* sozinho.
